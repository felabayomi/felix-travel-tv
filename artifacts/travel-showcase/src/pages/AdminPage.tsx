...
import { useNewsClipController } from '@/hooks/use-news-clip-controller';
...
function AdminDashboard() {
  ...
  // which fires the reset effect (setCurrentSnippetIndex(0))
  ...
  // so local view tracks any server-driven interlude/queue changes.
  useEffect(() => {
    const handleVisible = () => {
  ...
  // New from-scratch clip controller module: one authority for voice-end + fallback timing.
  useNewsClipController({
    voiceEnabled,
    queueAutoplay,
    playingArticleId,
    currentSnippetIndex,
    ...
    serverItemType,
    voiceRestartToken,
    autoPlaySeconds: AUTO_PLAY_SECONDS,
    voiceFallbackSeconds: VOICE_FALLBACK_SECONDS,
    speak,
    onAdvance: () => advanceRef.current(),
    getLiveState: () => ({
      playingArticleId: playingArticleIdRef.current,
      currentSnippetIndex: currentSnippetIndexRef.current,
      snippets: snippetsRef.current,
      queueAutoplay: queueAutoplayRef.current,
      serverItemType: serverItemTypeRef.current,
    }),
  });
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          await apiSetQueueAutoplay(next);
                          if (next) {
                            // Turning autoplay ON: bump restart token so controller can
                            // re-evaluate current clip immediately without waiting for index change.
