...
import { X, Plus, Settings2, Loader2, Eye, EyeOff, Lock } from 'lucide-react';
...
export function NewsAdminPanel({ onArticleAdded }: NewsAdminPanelProps) {
  ...
  const createMutation = useCreateArticle({
    mutation: {
      ...
      onError: (err: any) => {
        setAddError(err?.data?.error || 'Failed to process URL — check that it is a valid news article.');
      }
    }
  });
  ...
  const handleAddUrl = (e: React.FormEvent) => {
    ...
    setAddError('');
    createMutation.mutate({ data: { url: newUrl.trim() } });
  };

  return (
    <>
      {/* Gear trigger — 6 taps to open */}
      <button
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-border">
                <div>
                  <h2 className="text-xl font-display font-bold text-foreground">News Reader</h2>
                  <p className="text-xs text-muted-foreground mt-1">Add a news article URL to process it</p>
                </div>
              ...
              <div className="p-6">
                <form onSubmit={handleAddUrl} className="space-y-4">
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-muted-foreground/40 resize-none"
                    />
                  </div>
                  ...
                  {createMutation.isPending && (
                    <div className="bg-primary/10 border border-primary/20 rounded-xl px-4 py-3 space-y-1">
                      ...
                      <p className="text-[11px] text-primary/60">
                        Breaking into chapters and generating images. This takes 30–60 seconds.
                      </p>
                    </div>
                  )}
                ...
                <div className="mt-8 p-4 rounded-xl border border-dashed border-border text-xs text-muted-foreground space-y-1.5">
                  <p className="font-medium text-foreground/60">How it works</p>
                  <p>Paste any news article URL and the AI will:</p>
                  <ul className="space-y-1 pl-4 list-disc">
                    <li>Read the full article content</li>
                    <li>Break it into 5–10 story chapters</li>
                    <li>Generate a unique image for each chapter</li>
                    <li>Play them as a looping visual story</li>
                  </ul>
                </div>
              </div>
            </motion.div>
